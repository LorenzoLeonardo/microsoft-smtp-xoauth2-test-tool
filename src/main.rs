...
async fn auth_code_grant(client_id: &str, sender_email: &str) -> OAuth2Result<AccessToken> {
    let auth_code_grant = AuthCodeGrant::new(
        ClientId::new(client_id.to_string()),
        Some(ClientSecret::new(
            "V7G8Q~vKKR4cd6s~DiSa81htgDAIn58oc-Dy_dns".into(),
        )),
...
async fn main() -> OAuth2Result<()> {
    ...
    let sender_email = &args[3];
    let sender_name = &args[4];
    let receiver_email = &args[5];
    let receiver_name = &args[6];
    init_logger(args[7].as_str());

    let access_token = match OAuth2TokenGrantFlow::from(args[1].to_string()) {
        OAuth2TokenGrantFlow::AuthorizationCodeGrant => {
            auth_code_grant(client_id, sender_email).await?
        }
        OAuth2TokenGrantFlow::DeviceCodeFlow => device_code_flow(client_id, sender_email).await?,
    };
